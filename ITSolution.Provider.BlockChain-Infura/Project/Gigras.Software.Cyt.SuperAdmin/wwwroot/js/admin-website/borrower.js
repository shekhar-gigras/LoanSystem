    ...
    return {
        init: function () {
            ...
            if (tableElement) {
                ...
                searchInput = document.querySelector('[data-kt-category-table-filter="loan-trans-search"]');
                if (searchInput) {
                    searchInput.addEventListener("keyup", function (event) {
...
$(document).ready(function () {
    ...
    $(document).on("click", ".view-json", function () {
        ...
        $.get(`${basedataUrl}/get-user-data?id=${id}`, function (data) {
            ...
            else if (csc.toLowerCase() == "transactionlog") {
                table = createLoanTransDataTableFromJson(jsonData);
            }
            else {
...
function deleteRecord(id, module) {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            fetch(`/api/${module}/${id}`, {
                method: "POST", // Use DELETE method if the API expects it
                headers: {
...
function findDescriptionByFieldName(jsonArray, fieldName) {
    // Loop through the array to find the matching FieldName
    for (const item of jsonArray) {
        if (item.FieldName === fieldName) {
            return item.FieldDescription; // Return FieldDescription if FieldName matches
        }
    }
