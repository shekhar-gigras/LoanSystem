...
namespace Gigras.Software.Cyt.SuperAdmin.Controllers
{
    public class SAuthController : Controller
    {
        ...
        public async Task<IActionResult> Register(ITAdmin model, IFormFile avatarFile)
        {
            if (!ModelState.IsValid)
            {
                foreach (var key in ModelState.Keys)
            ...
            {
                if (!new[] { ".jpg", ".jpeg", ".png", ".gif" }.Contains(Path.GetExtension(avatarFile.FileName).ToLower()))
                {
                    ModelState.AddModelError("Avatar", "Invalid file type. Only images are allowed.");
                    return View(model);
                }
                ...
                var uniqueFileName = $"{Path.GetFileNameWithoutExtension(avatarFile.FileName)}_{Guid.NewGuid()}{Path.GetExtension(avatarFile.FileName)}";
                ...
                model.Avatar = $"/uploads/{uniqueFileName}";
            }

            // Save data to database
            model.IsActive = false;
            model.Phone = "9899054898";
            ...
            var emailBody = $"Please verify your email by clicking the link: <a href='{verificationLink}'>Verify Email</a>";
            await EmailHelper.SendEmailAsync(_configuration, model.Name!, model.Email!, "Email Verification", emailBody);
        ...
        public async Task<IActionResult> ForgotPassword(ITAdmin model)
        {
            try
            {
                ...
                var emailBody = $"Please reset your password by clicking the link: <a href='{resetLink}'>Verify Email</a>";
                await EmailHelper.SendEmailAsync(_configuration, objUser.Name!, objUser.Email!, "Forget Password Link", emailBody);
